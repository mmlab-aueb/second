...
class CLPKC_RP03():
    ...
    def sign(self, params, SA, M):
        a = group.random(ZR)
        r = pair(a*params['P'], params['P'])
        u =  h.hashToZr(M, r)
        U = u*SA + a*params['P']
        return {'U':U, 'u':u}
    
    def verify(self, params, M, signature, ID, PA):
        if pair(PA['XA'], params['P0']) != pair(PA['YA'], params['P']):
            if debug: print("Invalid public key")
            return False
        QA = group.hash(ID, G1)
        _r1 = pair(signature['U'], params['P'])
        _r2 = pair(QA, -PA['YA']) ** signature['u']
        r = _r1*_r2
        return signature['u'] == h.hashToZr(M, r)
